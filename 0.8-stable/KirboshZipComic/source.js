...
var _Sources = (() => {
  ...
  // src/KirboshZipComic/KirboshZipComic.ts
  var KirboshZipComic_exports = {};
  __export(KirboshZipComic_exports, {
    KirboshZipComic: () => KirboshZipComic,
    KirboshZipComicInfo: () => KirboshZipComicInfo
  });
  ...
  // src/KirboshZipComic/KirboshZipComicParser.ts
  var ZIPCOMIC_DOMAIN = "https://www.zipcomic.com";
  function absoluteHttpsUrl(rawUrl) {
  ...
  function pathFromUrl(rawUrl) {
    return rawUrl.trim().replace(/^https?:\/\/[^/]+/i, "").replace(/[?#].*$/, "").replace(/^\/+|\/+$/g, "");
  }
  ...
  function parseChapterId(rawUrl) {
    const path = pathFromUrl(rawUrl);
  ...
  function imageFrom(element) {
    return absoluteHttpsUrl(
      element.attr("data-src") ?? element.attr("data-lazy-src") ?? element.attr("data-original") ?? element.attr("src") ?? ""
    );
  }
  function parseCards(html3) {
    const $2 = load(html3);
    const results = [];
    const seen = /* @__PURE__ */ new Set();
    $2("img.img-responsive").each((_, element) => {
      const imageElement = $2(element);
  ...
  function zipComicSearchUrl(title, page) {
  ...
  function hasNextPage(html3, currentPage) {
    const $2 = load(html3);
    return $2(".pagination a").toArray().some((element) => {
      const href = $2(element).attr("href") ?? "";
      const page = Number.parseInt(href.match(/[?&]p=(\d+)/)?.[1] ?? "", 10);
      return !Number.isNaN(page) && page > currentPage;
    });
  }
  function labelledValue($2, names) {
    let result = { text: "", links: [] };
    $2("strong.text-success").each((_, element) => {
      if (result.text || result.links.length) return;
      const label = $2(element).text().replace(":", "").trim().toLowerCase();
      if (!names.includes(label)) return;
      const container = $2(element).parent();
      result = {
        text: container.text().replace($2(element).text(), "").replace(/\s+/g, " ").trim(),
        links: container.find("a").toArray().map((link) => $2(link).text().trim()).filter(Boolean)
      };
    });
    return result;
  }
  function parseDetails(html3, fallbackTitle = "") {
    const $2 = load(html3);
    const title = $2("h1").first().text().trim() || fallbackTitle;
    if (!title) throw new Error("ZipComic returned a title page without a comic title");
    const statusText = labelledValue($2, ["status"]).text.toLowerCase();
    const status = statusText.includes("ongoing") ? "ONGOING" : statusText.includes("complete") ? "COMPLETED" : "UNKNOWN";
    const genres = labelledValue($2, ["genre", "genres"]).links;
    return {
      title,
      image: imageFrom($2('img[src*="cover"], img[data-src*="cover"]').first()),
      status,
      author: labelledValue($2, ["author", "writer"]).text,
      artist: labelledValue($2, ["artist", "artis"]).text,
      genres
    };
  }
  function parseChapters(html3) {
    ...
    const chapters = [];
    const seen = /* @__PURE__ */ new Set();
    $2("table tr").each((index2, element) => {
      const row = $2(element);
      ...
      chapters.push({
        id,
        title: title || `Issue ${Number.isNaN(number) ? index2 + 1 : number}`,
        number: Number.isNaN(number) ? Number.isNaN(order) ? index2 + 1 : order : number,
        sortingIndex: Number.isNaN(order) ? index2 : order
      });
    });
    return chapters;
  }
  function parsePages(html3) {
    const $2 = load(html3);
    const pages = [];
    const seen = /* @__PURE__ */ new Set();
    $2("#images img").each((_, element) => {
      const page = imageFrom($2(element));
  ...
  function looksLikeCloudflareChallenge(html3) {
    ...
    return lower.includes("cf-chl-") || lower.includes("just a moment") || lower.includes("challenges.cloudflare.com") || lower.includes("window.performance") && lower.includes("crypto.subtle");
  }

  // src/KirboshZipComic/KirboshZipComic.ts
  var KirboshZipComicInfo = {
    version: "1.0.0",
    name: "ZipComic",
  ...
  var KirboshZipComic = class {
    ...
    async requestHtml(url) {
      ...
      if (!html3.trim()) throw new Error(`ZipComic returned an empty response for ${url}`);
      return html3;
    }
    partialManga(card) {
    ...
    resultsPage(html3, page, collectedIds = []) {
      const seen = new Set(collectedIds);
      const cards = parseCards(html3).filter((card) => {
    ...
    async getHomePageSections(sectionCallback) {
      const items = parseCards(await this.requestHtml(`${ZIPCOMIC_DOMAIN}/`)).map(
        (card) => this.partialManga(card)
      );
      sectionCallback(
    ...
    async getSearchResults(query, metadata) {
      ...
      const html3 = await this.requestHtml(zipComicSearchUrl(query.title ?? "", page));
      return this.resultsPage(html3, page, metadata?.collectedIds);
    }
    async getViewMoreItems(sectionId) {
  ...
  return __toCommonJS(KirboshZipComic_exports);
})();
