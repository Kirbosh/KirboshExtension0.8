...
var _Sources = (() => {
  ...
  var KirboshBatCave_exports = {};
  __export(KirboshBatCave_exports, {
    KirboshBatCave: () => KirboshBatCave,
    KirboshBatCaveInfo: () => KirboshBatCaveInfo
  });
  ...
  // src/KirboshBatCave/KirboshBatCaveParser.ts
  var BATCAVE_DOMAIN = "https://batcave.biz";
  var SELECTORS = {
  ...
  function parseSearchResults(html3) {
    const $2 = load(html3);
    const cards = [];
    $2(SELECTORS.searchCard).each((_, element) => {
      const unit = $2(element);
      const titleLink = unit.find(SELECTORS.searchTitle).first();
      cards.push({
        id: parseMangaId(titleLink.attr("href") ?? ""),
        image: lazyImage(
          unit.find(SELECTORS.searchImage).first().attr("data-src") ?? unit.find(SELECTORS.searchImage).first().attr("data-lazy-src") ?? unit.find(SELECTORS.searchImage).first().attr("data-original") ?? unit.find(SELECTORS.searchImage).first().attr("src") ?? ""
        ),
        title: titleLink.text().trim(),
        subtitle: unit.find(".readed__info li:last-child").text().replace(/last issue:\s*/i, "").trim()
      });
    });
    return uniqueCards(cards);
  }
  function parsePopular(html3) {
    const $2 = load(html3);
    const cards = [];
    $2(SELECTORS.popularCard).each((_, element) => {
      const unit = $2(element);
      ...
      cards.push({
        ...
        image: lazyImage(
          unit.find(SELECTORS.popularImage).first().attr("data-src") ?? unit.find(SELECTORS.popularImage).first().attr("src") ?? ""
        ),
        title: unit.find(SELECTORS.popularTitle).first().text().trim(),
        subtitle: rating ? `Rating: ${rating}` : void 0
      });
    });
    return uniqueCards(cards);
  }
  function parseLatest(html3) {
    const $2 = load(html3);
    const cards = [];
    $2(SELECTORS.latestCard).each((_, element) => {
      const unit = $2(element);
      ...
      cards.push({
        id: parseMangaId(titleLink.attr("href") ?? ""),
        image: lazyImage(
          unit.find(SELECTORS.latestImage).first().attr("data-src") ?? unit.find(SELECTORS.latestImage).first().attr("src") ?? ""
        ),
        title,
  ...
  function hasNextPage(html3, currentPage) {
    ...
    return $2(".pagination a, .pagination__pages a").toArray().some((element) => {
      ...
      return text3.includes("\xBB") || !Number.isNaN(pageFromText) && pageFromText > currentPage || !Number.isNaN(pageFromHref) && pageFromHref > currentPage;
    });
  }
  function metadataValues($2, labels) {
    const row = $2(SELECTORS.metadataRow).filter((_, element) => {
      const text4 = $2(element).text().toLowerCase();
      return labels.some((label) => text4.includes(label.toLowerCase()));
    }).first();
    const linkedValues = row.find("a").toArray().map((element) => $2(element).text().trim()).filter(Boolean);
    if (linkedValues.length > 0) return linkedValues;
    const text3 = row.text().replace(/^[^:]+:\s*/, "").trim();
    return text3 ? text3.split(",").map((value) => value.trim()).filter(Boolean) : [];
  }
  function parseDetails(html3) {
    const $2 = load(html3);
    const title = $2(SELECTORS.title).first().text().trim();
    if (!title) throw new Error("BatCave returned a title page without a comic title");
    const statusText = metadataValues($2, ["release type", "status"]).join(" ").toLowerCase();
    const status = statusText.includes("completed") ? "COMPLETED" : statusText.includes("ongoing") ? "ONGOING" : "UNKNOWN";
    const ratingText = $2(SELECTORS.rating).first().text();
    const ratingValue = Number.parseFloat(ratingText.match(/\d+(?:\.\d+)?/)?.[0] ?? "");
    return {
      title,
      image: lazyImage(
        $2(SELECTORS.cover).first().attr("data-src") ?? $2(SELECTORS.cover).first().attr("src") ?? ""
      ),
      description: $2(SELECTORS.description).first().text().trim(),
      rating: Number.isNaN(ratingValue) ? void 0 : ratingValue,
      status,
      authors: metadataValues($2, ["writer", "author"]),
      artists: metadataValues($2, ["artist"]),
      publishers: metadataValues($2, ["publisher"]),
      genres: $2(SELECTORS.genre).toArray().map((element) => $2(element).text().trim()).filter(Boolean)
    };
  }
  function extractWindowData(html3) {
    const markerIndex = html3.indexOf("window.__DATA__");
    if (markerIndex < 0) throw new Error("BatCave response does not contain reader data");
    const start = html3.indexOf("{", markerIndex);
    if (start < 0) throw new Error("BatCave reader data is malformed");
    let depth = 0;
    let inString = false;
    let escaped = false;
    for (let index2 = start; index2 < html3.length; index2 += 1) {
      const character = html3[index2];
      if (inString) {
      ...
      if (character === '"') inString = true;
        ...
        if (depth === 0) {
          try {
            return JSON.parse(html3.slice(start, index2 + 1));
          } catch {
  ...
  function parseChapters(html3) {
    const data2 = extractWindowData(html3);
    const chapters = (data2.chapters ?? []).map((chapter) => {
      const id = String(chapter.id ?? "").trim();
      const number = Number(chapter.posi);
      if (!id || !Number.isFinite(number)) return void 0;
      return {
        id,
        number,
    ...
    }).filter((chapter) => chapter !== void 0);
    chapters.sort((left, right) => right.number - left.number);
    return chapters;
  }
  function parseReaderData(html3) {
    const data2 = extractWindowData(html3);
    const seen = /* @__PURE__ */ new Set();
    const images = (data2.images ?? []).map((image) => absoluteHttpsUrl(image)).filter((image) => {
      if (!image || seen.has(image)) return false;
      seen.add(image);
      return true;
    });
    return { images, usesAjax: data2.rdr_ajax === true };
  }
  ...
  var KirboshBatCaveInfo = {
    ...
    sourceTags: [
      { text: "Western comics", type: import_types2.BadgeColor.BLUE },
      { text: "English", type: import_types2.BadgeColor.GREEN }
    ]
  };
  var KirboshBatCave = class {
    constructor() {
    ...
    async getMangaDetails(mangaId) {
      ...
      const tags = details.genres.length ? [
        {
          id: "genres",
          label: "Genres",
    ...
    async getViewMoreItems(sectionId, metadata) {
      ...
      const html3 = await this.requestHtml(url);
      const cards = latest ? parseLatest(html3) : parseSearchResults(html3);
      return this.resultsPage(html3, page, metadata?.collectedIds, cards);
    }
    resultsPage(html3, page, collectedIds = [], parsedCards = parseSearchResults(html3)) {
      ...
      return App.createPagedResults({
        ...
        metadata: hasNextPage(html3, page) ? { page: page + 1, collectedIds: Array.from(seen) } : void 0
      });
    }
  ...
  return __toCommonJS(KirboshBatCave_exports);
})();
