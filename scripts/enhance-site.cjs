...
if (manifest.sources.length !== 1) {
    throw new Error(`Expected exactly one published source, found ${manifest.sources.length}`)
}
...
if (!sourceListItem.test(html)) throw new Error('The standard source list was not found')

const rating = source.contentRating.charAt(0) + source.contentRating.slice(1).toLowerCase()
html = html.replace(
    sourceListItem,
    `<li class="sourceCard"><img class="sourceIcon" src="${source.id}/includes/${source.icon}" alt="${source.name} icon"><span class="sourceIdentity"><strong>${source.name}</strong><small>Version ${source.version} · ${rating} · English</small></span>$1</li>`,
)
html = html.replace(
    '</style>',
